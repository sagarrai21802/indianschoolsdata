"""YellowSlate Scraper - Fixed deduplication."""
...
async def get_all_slugs(city: str) -> list[str]:
    """Get all school slugs with proper scroll pagination."""
    async with async_playwright() as p:
        ...
        # Scroll and track unique slugs
        all_slugs = {}  # slug -> True (preserves insertion order)
        
        for scroll_num in range(20):
            await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
            await asyncio.sleep(2)
            
            # Extract slugs from page
            ...
            print(f"    Scroll {scroll_num + 1}: {current} unique schools")
...
def save_results(city: str, results: list):
    ...
    json_path = f"{RESULTS_DIR}/v2_{city}_{ts}.json"
    ...
    if flat:
        csv_path = f"{RESULTS_DIR}/v2_{city}_{ts}.csv"
...
async def scrape_city(city: str):
    ...
    async with async_playwright() as p:
        ...
        for i, slug in enumerate(pending):
            print(f"  [{i+1}/{len(pending)}] {slug[:50]}...")
            try:
